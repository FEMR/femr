    ...
    public List<ValidationError> validate(){
        ...
        if(newPassword.length() > 0)
        {
            if(newPassword.length() < 8 || !hasUppercase.matcher(newPassword).find()
                    ...
                    || !hasLowercase.matcher(newPassword).find()) //AJ Saclayan Password Constraints
                errors.add(new ValidationError("newPassword",
                        "password must have at least one Uppercase, one lowercase, one digit and 8 characters long.");
        }
