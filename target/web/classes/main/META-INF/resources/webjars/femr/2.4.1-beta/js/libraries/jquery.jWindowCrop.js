...
(function($){
	...
	$.jWindowCrop = function(image, options){
		...
		base.init = function(){
            ...
            $('#' + options.zoomInId).click('click.'+base.namespace, base.zoomIn);
            $('#' + options.zoomOutId).click('click.'+base.namespace, base.zoomOut);
		...
		function handleMouseMove(event) {
			if(base.isDragging) {
				...
				storeFocalPoint();
				updateResult();
                base.$frame.find('.jwc_controls').show();
			}
		}
        ...
        function hidePanel() {
            if(base.options.smartControls) base.$frame.find('.jwc_controls').delay(3000).fadeOut('fast');
        }
